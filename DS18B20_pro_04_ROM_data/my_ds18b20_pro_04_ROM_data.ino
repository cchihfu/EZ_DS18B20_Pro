...
#define Read_ROM 			0x33	//用於1對1時，省略每次作ROM序號確認程序
...
float getROM()
{
	...
	while(!CommandReset());      	//重啟成功，再進行動作
	SendCommand(Read_ROM);		   //主機下0x33命令
	for(int i = 0; i < 8; ++i)
	{
		ROM_data[i]=ReceiveData();
	}
	PrintROM();
}
void PrintROM() {
	Serial.println("DS18b20's ROM Data");

	Serial.print("1.Product Type->");
	Serial.print("0x");
	Serial.print(ROM_data[0],HEX);
	Serial.println("");


	Serial.print("2.ID No->");
	for(int i = 1; i < 7; ++i)
	{
		Serial.print("0x");
		Serial.print(ROM_data[i],HEX);
		Serial.print(", ");
	}
	Serial.print("3.CRC No->");
	Serial.print("0x");
	Serial.print(ROM_data[8],HEX);
	Serial.println("");
}
